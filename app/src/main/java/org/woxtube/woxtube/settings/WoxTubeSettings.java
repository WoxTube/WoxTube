 ...
 * WoxTubeSettings.java is part of WoxTube.
 *
 ...
 */
public final class WoxTubeSettings {
    private WoxTubeSettings() { }
    ...
    private static void saveDefaultDirectory(final Context context, final int keyID,
        ...
        if (!useStorageAccessFramework(context)) {
            ...
            spEditor.putString(key, getWoxTubeChildFolderPathForDir(getDir(defaultDirectoryName)));
            spEditor.apply();
        }
    }
    ...
    private static String getWoxTubeChildFolderPathForDir(final File dir) {
        return new File(dir, "WoxTube").toURI().toString();
    }
